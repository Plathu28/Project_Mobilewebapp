      ...
      <div class="pb-28">
        ...
        <div class="flex items-center px-4 pt-12 pb-4">
          ...
          <button v-if="notifStore.unreadCount > 0" @click="notifStore.markAllRead()"
            class="ml-auto text-xs font-semibold text-blue-500 active:opacity-60">
            Mark all read
          </button>
        </div>
        ...
        <div class="flex border-b border-gray-200 mx-4 mb-1">
          <button class="flex-1 py-3 text-sm font-bold tracking-wide transition-colors"
            :class="activeTab === 'all' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400'"
            @click="activeTab = 'all'">ALL</button>
          <button class="flex-1 py-3 text-sm font-bold tracking-wide transition-colors"
            :class="activeTab === 'unread' ? 'text-gray-900 border-b-2 border-gray-900' : 'text-gray-400'"
            @click="activeTab = 'unread'">
            UNREAD<span v-if="notifStore.unreadCount > 0"> ({{ notifStore.unreadCount }})</span>
          </button>
        </div>
        ...
        <div v-else>
          <div v-for="item in displayList" :key="item.id"
            ...
            :class="notifStore.isRead(item.id) ? 'bg-white' : 'bg-blue-50'"
            @click="notifStore.markRead(item.id)">

            <div class="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0"
              :class="notifStore.isRead(item.id) ? 'bg-gray-100' : 'bg-blue-100'">
              <ion-icon :icon="notificationsOutline" class="text-2xl"
                :class="notifStore.isRead(item.id) ? 'text-gray-400' : 'text-blue-400'"></ion-icon>
            </div>
            ...
            <div v-if="!notifStore.isRead(item.id)" class="w-2.5 h-2.5 rounded-full bg-blue-400 flex-shrink-0"></div>
          </div>
        </div>
...
const displayList = computed(() => {
  if (activeTab.value === 'unread') {
    return allNotifications.value.filter((n) => !notifStore.isRead(n.id));
  }
...
<style scoped>
.notif-content { --background: #ffffff; }
</style>