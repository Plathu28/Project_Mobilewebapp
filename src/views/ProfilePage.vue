      ...
      <div class="pb-28">
        ...
        <div class="flex items-center px-4 pt-12 pb-4">
          ...
          <div class="ml-auto flex items-center gap-2">
            ...
            >
              ...
              <!-- ✅ ใช้ notifStore.unreadCount แทน -->
              <span v-if="notifStore.unreadCount > 0" class="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-white text-xs font-bold flex items-center justify-center">
        ...
        <div class="flex flex-col items-center py-6 px-4">
          <div class="relative mb-4">
            <div class="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg">
              ...
              <div v-if="avatarUploading" class="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center">
                <ion-spinner name="crescent" class="text-white w-6 h-6"></ion-spinner>
              </div>
            </div>
            <button @click="showAvatarOptions" class="absolute bottom-0 right-0 w-7 h-7 bg-gray-900 rounded-full flex items-center justify-center border-2 border-white active:scale-90 transition-transform">
        ...
        <div class="mx-4 mb-6">
          ...
          <div v-if="!classroomConnected" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <p class="text-gray-600 text-sm mb-4">Connect Google Classroom to automatically import your assignments as tasks.</p>
            <button @click="handleClassroomAuth" :disabled="classroomLoading"
          ...
          <div v-else class="space-y-3">
            <div class="bg-green-50 rounded-2xl border border-green-200 p-4">
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-green-600 text-xs mt-0.5">{{ connectedCourses.length }} course{{ connectedCourses.length !== 1 ? 's' : '' }} synced</p>
                </div>
                <button @click="handleSyncNow" :disabled="classroomLoading"
            ...
            <p v-if="lastSyncTime" class="text-xs text-gray-400 px-1">Last synced: {{ lastSyncTime }} · {{ classroomTasks.length }} tasks imported</p>

            <div v-if="connectedCourses.length > 0" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              <div v-for="(course, index) in connectedCourses" :key="course.id"
                class="flex items-center gap-3 px-4 py-3" :class="{ 'border-t border-gray-100': index > 0 }">
                ...
                <span class="text-xs text-gray-400 flex-shrink-0">{{ courseTaskCounts[course.name] || 0 }} tasks</span>
              </div>
            </div>
            ...
            <div v-if="classroomTasks.length > 0">
              ...
              <div class="space-y-2">
                <div v-for="task in classroomTasks.slice(0, showAllClassroom ? 999 : 5)" :key="task.id"
                  class="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm border border-gray-100">
                  <button class="w-5 h-5 rounded-full border-2 border-blue-400 flex-shrink-0 flex items-center justify-center active:bg-green-100 transition-colors"
                    @click="handleComplete(task.id)"></button>
                  <div class="flex-1 min-w-0">
                    <p class="font-semibold text-sm text-gray-800 truncate">{{ task.title.replace('[Classroom] ', '') }}</p>
                ...
                <button v-if="classroomTasks.length > 5" @click="showAllClassroom = !showAllClassroom"
            ...
            <button @click="handleDisconnect" class="w-full py-3 text-center text-red-400 text-sm font-medium active:text-red-600 transition-colors mt-2">
        ...
        <div class="mx-4 mb-6">
          ...
          <div v-if="favoriteLabels.length === 0" class="text-sm text-gray-400">No favorite labels yet.</div>
          <div class="flex gap-2 flex-wrap">
            <span v-for="label in favoriteLabels" :key="label.name"
    ...
    <transition name="sheet">
      <div v-if="showSettings" class="fixed inset-0 z-50 flex flex-col justify-end">
        ...
        <div class="relative bg-white rounded-t-3xl px-4 pt-4 pb-10 z-10">
          ...
          <button class="w-full py-4 rounded-2xl bg-red-50 text-red-500 font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform"
            @click="handleSignOut">
...
import {
  authorizeClassroom, isClassroomAuthorized, revokeClassroom,
  pullClassroomAssignments, fetchCourses, type ClassroomTask,
...
async function showAvatarOptions() {
  const buttons: any[] = [
    { text: 'Choose from Gallery', icon: imageOutline, handler: () => fileInput.value?.click() },
  ];
  if (currentUser.value?.photoURL || customAvatarUrl.value) {
    buttons.push({ text: 'Remove Photo', icon: trashOutline, role: 'destructive', handler: () => removeAvatar() });
  }
  buttons.push({ text: 'Cancel', role: 'cancel' });
  const actionSheet = await actionSheetController.create({ header: 'Profile Photo', buttons });
...
async function handleFileSelected(e: Event) {
  ...
  if (!file.type.startsWith('image/')) {
    const toast = await toastController.create({ message: '❌ Please select an image file', duration: 2000, position: 'bottom', color: 'danger' });
    await toast.present(); return;
  }
  if (file.size > 5 * 1024 * 1024) {
    const toast = await toastController.create({ message: '❌ Image must be under 5MB', duration: 2000, position: 'bottom', color: 'danger' });
    await toast.present(); return;
  }
...
async function uploadAvatar(file: File) {
  ...
  try {
    ...
    const fileRef = storageRef(storage, `avatars/${currentUser.value.uid}/profile.${ext}`);
    await uploadBytes(fileRef, file, { contentType: file.type });
    const downloadUrl = await getDownloadURL(fileRef);
    await updateProfile(currentUser.value, { photoURL: downloadUrl });
    customAvatarUrl.value = downloadUrl;
    const toast = await toastController.create({ message: '✅ Profile photo updated!', duration: 1500, position: 'bottom', color: 'success' });
    await toast.present();
  } catch (err: any) {
    const toast = await toastController.create({ message: `❌ Upload failed: ${err.message}`, duration: 2500, position: 'bottom', color: 'danger' });
    await toast.present();
  } finally {
...
async function removeAvatar() {
  ...
  try {
    ...
    const toast = await toastController.create({ message: '✅ Profile photo removed', duration: 1500, position: 'bottom', color: 'success' });
    await toast.present();
  } catch (err: any) {
...
const classroomTasks = computed(() => store.activeTasks.filter((t) => t.title.startsWith('[Classroom]')));

const courseTaskCounts = computed(() => {
  const counts: Record<string, number> = {};
  for (const task of classroomTasks.value) {
    const courseName = task.subtasks?.[0]?.title;
    if (courseName) counts[courseName] = (counts[courseName] || 0) + 1;
  }
  return counts;
});

const courseColors = ['#4285F4', '#34A853', '#EA4335', '#FBBC05', '#8E24AA', '#00ACC1', '#FF7043', '#5C6BC0', '#26A69A', '#EC407A'];
function getCourseColor(index: number): string { return courseColors[index % courseColors.length]; }

function normalizeTitle(title: string): string { return title.replace(/^\[Classroom\]\s*/i, '').trim().toLowerCase(); }
...
function findMatchingTask(assignmentTitle: string, courseName: string) {
  ...
  for (const task of store.tasks) {
    ...
    if (matchesCourse) return { found: true, isActive: !task.completed, isCompleted: task.completed, taskId: task.id };
  }
...
async function askReAddTask(title: string, courseName: string): Promise<boolean> {
  return new Promise(async (resolve) => {
    const alert = await alertController.create({
      header: '⚠️ Not Turned In Yet', subHeader: title,
      ...
      buttons: [
        { text: 'No, Skip', role: 'cancel', handler: () => resolve(false) },
        { text: 'Yes, Add Back', handler: () => resolve(true) },
      ],
    });
    await alert.present();
  });
...
onMounted(async () => {
  if (currentUser.value?.photoURL) customAvatarUrl.value = currentUser.value.photoURL;
...
async function handleClassroomAuth() {
  ...
  try {
    ...
    connectedCourses.value = await fetchCourses();
    await syncClassroomTasks();
    const toast = await toastController.create({ message: `✅ Connected! Found ${connectedCourses.value.length} courses`, duration: 2000, position: 'bottom', color: 'success' });
    await toast.present();
  } catch (err: any) {
    const toast = await toastController.create({ message: `❌ ${err.message || 'Failed to connect'}`, duration: 2500, position: 'bottom', color: 'danger' });
    await toast.present();
...
async function syncClassroomTasks() {
  ...
  try {
    ...
    let newCount = 0, reAddCount = 0, skippedCount = 0;
    for (const a of assignments) {
      const match = findMatchingTask(a.title, a.courseName);
      if (match.found && match.isActive) { skippedCount++; continue; }
      if (match.found && match.isCompleted) {
        const wantReAdd = await askReAddTask(a.title, a.courseName);
        if (!wantReAdd) { skippedCount++; continue; }
        reAddCount++;
      }
      await store.addTask({
        title: `[Classroom] ${a.title}`, category: 'Work',
        date: a.dueDate, startTime: a.dueTime, endTime: null, duration: null,
        priority: a.dueDate ? 'medium' : 'low', routine: null, completed: false, labels: [],
        subtasks: [{ id: Date.now().toString() + Math.random(), title: a.courseName, completed: false }],
      });
      if (!match.found) newCount++;
    }
    lastSyncTime.value = new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });
    const parts = [];
    if (newCount > 0) parts.push(`${newCount} new`);
    if (reAddCount > 0) parts.push(`${reAddCount} re-added`);
    const toast = await toastController.create({
      message: parts.length > 0 ? `📚 ${parts.join(', ')} assignment${(newCount + reAddCount) !== 1 ? 's' : ''}` : '✅ Everything is up to date',
      duration: 2000, position: 'bottom', color: parts.length > 0 ? 'primary' : undefined,
    });
    await toast.present();
  } catch (err: any) {
    if (err.message?.includes('expired') || err.message?.includes('authorize')) { classroomConnected.value = false; revokeClassroom(); }
    const toast = await toastController.create({ message: `❌ Sync failed: ${err.message || 'Unknown error'}`, duration: 2000, position: 'bottom', color: 'danger' });
    await toast.present();
  } finally { classroomLoading.value = false; }
}

async function handleSyncNow() { await syncClassroomTasks(); }
...
function handleDisconnect() { revokeClassroom(); classroomConnected.value = false; connectedCourses.value = []; lastSyncTime.value = null; }
...
const getThemeFromColor = (ionicColor: string) => {
  const themes: Record<string, { bg: string; text: string }> = {
    primary: { bg: 'bg-blue-100', text: 'text-blue-700' }, secondary: { bg: 'bg-cyan-100', text: 'text-cyan-700' },
    tertiary: { bg: 'bg-purple-100', text: 'text-purple-700' }, success: { bg: 'bg-green-100', text: 'text-green-700' },
    warning: { bg: 'bg-amber-100', text: 'text-amber-700' }, danger: { bg: 'bg-red-100', text: 'text-red-500' },
...
onMounted(() => {
  onAuthStateChanged(auth, (user) => {
    if (user) {
      ...
      const q = query(collection(db, 'labels'), where('userId', '==', user.uid), where('isFavorite', '==', true));
...
interface MenuItem { label: string; icon: string; action: (() => void) | null; }
